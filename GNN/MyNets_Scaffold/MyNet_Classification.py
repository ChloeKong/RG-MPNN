...
# set raw_name for scaffold split
train_dataset_params =  {key: value for key,value in dataset_params.items()}
train_dataset_params['raw_name'] = train_dataset_params['raw_name'][:-4] + '_ScafflodSplit_train.csv'

test_dataset_params =  {key: value for key,value in dataset_params.items()}
test_dataset_params['raw_name'] = test_dataset_params['raw_name'][:-4] + '_ScafflodSplit_test.csv'

valid_dataset_params =  {key: value for key,value in dataset_params.items()}
valid_dataset_params['raw_name'] = valid_dataset_params['raw_name'][:-4] + '_ScafflodSplit_valid.csv'

# seperate scaffold train、valid、test
train_dataset = Raw_Dataset(**train_dataset_params)
train_rg_dataset = ReduceGraph_Dataset(**train_dataset_params)

test_dataset = Raw_Dataset(**test_dataset_params)
test_rg_dataset = ReduceGraph_Dataset(**test_dataset_params)

valid_dataset = Raw_Dataset(**valid_dataset_params)
valid_rg_dataset = ReduceGraph_Dataset(**valid_dataset_params)

print('train_rg_dataset', train_rg_dataset)
print('train_dataset', train_dataset)

print('test_rg_dataset', test_rg_dataset)
print('test_dataset', test_dataset)

print('valid_rg_dataset', valid_rg_dataset)
print('valid_dataset', valid_dataset)


batch = option['batch']
train_dataset.data.y = train_dataset.data.y.long() 
test_dataset.data.y = test_dataset.data.y.long()
valid_dataset.data.y = valid_dataset.data.y.long()

# train_dataloader = DataLoader(train_dataset, batch_size=batch) # 80%
val_dataloader = DataLoader(valid_dataset, batch_size=batch) # 10%
test_dataloader = DataLoader(test_dataset, batch_size=batch) # 10%

# train_rg_dataloader = DataLoader(train_rg_dataset, batch_size=batch) # 80%
val_rg_dataloader = DataLoader(valid_rg_dataset, batch_size=batch) # 10%
test_rg_dataloader = DataLoader(test_rg_dataset, batch_size=batch) # 10%


# read dataset dim
atom_dim = train_dataset[0].x.shape[1]
...
for epoch in range(epochs):                 
    ...
    state_dict, loss, train_pair = train(epoch, train_dataloader, train_rg_dataloader) 
